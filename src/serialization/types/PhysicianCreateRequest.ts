// This file was auto-generated by Fern from our API Definition.

import type * as Junction from "../../api/index.js";
import * as core from "../../core/index.js";
import type * as serializers from "../index.js";
import { PhysicianCreateRequestSignatureImage } from "./PhysicianCreateRequestSignatureImage.js";

export const PhysicianCreateRequest: core.serialization.ObjectSchema<
    serializers.PhysicianCreateRequest.Raw,
    Junction.PhysicianCreateRequest
> = core.serialization.object({
    firstName: core.serialization.property("first_name", core.serialization.string()),
    lastName: core.serialization.property("last_name", core.serialization.string()),
    email: core.serialization.string().optionalNullable(),
    npi: core.serialization.string(),
    licensedStates: core.serialization.property(
        "licensed_states",
        core.serialization.list(core.serialization.string()).optional(),
    ),
    signatureImage: core.serialization.property(
        "signature_image",
        PhysicianCreateRequestSignatureImage.optionalNullable(),
    ),
});

export declare namespace PhysicianCreateRequest {
    export interface Raw {
        first_name: string;
        last_name: string;
        email?: (string | null | undefined) | null;
        npi: string;
        licensed_states?: string[] | null;
        signature_image?: (PhysicianCreateRequestSignatureImage.Raw | null | undefined) | null;
    }
}
