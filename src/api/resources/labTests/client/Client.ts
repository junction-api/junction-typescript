...
export class LabTestsClient {
     ...
     *
     * @param {Junction.GetOrderTrackingLabTestsRequest} request
     * @param {LabTestsClient.RequestOptions} requestOptions - Request-specific configuration.
     *
     * @throws {@link Junction.UnprocessableEntityError}
     *
    ...
    public getOrderTracking(
        ...
        requestOptions?: LabTestsClient.RequestOptions,
    ): core.HttpResponsePromise<Junction.OrderTracking> {
        return core.HttpResponsePromise.fromPromise(this.__getOrderTracking(request, requestOptions));
    }

    private async __getOrderTracking(
        request: Junction.GetOrderTrackingLabTestsRequest,
        requestOptions?: LabTestsClient.RequestOptions,
    ): Promise<core.WithRawResponse<Junction.OrderTracking>> {
        const { orderId } = request;
        const _authRequest: core.AuthRequest = await this._options.authProvider.getAuthRequest();
        const _headers: core.Fetcher.Args["headers"] = mergeHeaders(
            _authRequest.headers,
        ...
        const _response = await core.fetcher({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.JunctionEnvironment.Production,
                `v3/order/${core.url.encodePathParam(orderId)}/tracking`,
            ),
            method: "GET",
            headers: _headers,
            queryString: core.url.queryBuilder().mergeAdditional(requestOptions?.queryParams).build(),
        ...
        if (_response.ok) {
            return {
                data: serializers.OrderTracking.parseOrThrow(_response.body, {
                    unrecognizedObjectKeys: "passthrough",
                    allowUnrecognizedUnionMembers: true,
                    allowUnrecognizedEnumValues: true,
                    breadcrumbsPrefix: ["response"],
        ...
        return handleNonStatusCodeError(_response.error, _response.rawResponse, "GET", "/v3/order/{order_id}/tracking");
    }
