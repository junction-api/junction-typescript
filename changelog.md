## 1.1.0 - 2026-05-15
### Added
* **`LabReportResultLoincMatchStatus`** — new enum type with values `auto_match`, `needs_review`, and `no_match` indicating the LOINC matching status of a lab report result.
* **`LabReportResult.loincMatchStatus`** — new optional field on `LabReportResult` exposing the LOINC match status for each result.
